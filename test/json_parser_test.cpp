...
TEST(ParserTest, Bool_Should_Parse)
{
    std::string str("true");
    numeric_handler<bool> handler;
    auto parser = json::detail::make_shared_parser_impl(str.begin(), str.end(), handler);
    parser->parse_true();
    EXPECT_EQ(true, handler.actual);

    str = "false";
    parser = json::detail::make_shared_parser_impl(str.begin(), str.end(), handler);
    parser->parse_false();
    EXPECT_EQ(false, handler.actual);
}


TEST(ParserTest, Integers_Should_ParseLimits)
{    
...
TEST(ParserTest, Integers_Should_ThrowOutsideOfLimits)
{
    ...
    EXPECT_ANY_THROW(parse_integer<std::uint64_t>("18446744073709551616")); // 2 ^ 64 + 1
    EXPECT_ANY_THROW(parse_integer<std::uint64_t>("-1"));
    
    EXPECT_ANY_THROW(parse_integer<std::int64_t>("9223372036854775809"));  // 2 ^ 63 + 1
    EXPECT_ANY_THROW(parse_integer<std::int64_t>("-9223372036854775810")); // -2 ^ 63 + 1
}


TEST(ParserTest, Integers_Should_Parse)
{
    ...
    auto parser = json::detail::make_parser_impl(str.cbegin(), str.cend(), handler);
    parser.parse_number();
    EXPECT_EQ(3808159498143417627, handler.actual);

    EXPECT_EQ(-5514085325291784739, parse_integer<long long>("-5514085325291784739"));
    EXPECT_EQ(3808159498143417627, parse_integer<long long>("3808159498143417627"));
}


TEST(ParserTest, ParseRealHandler)
{    
    ...
    EXPECT_EQ(-1.79769e+308, parse_real<double>("-1.79769e+308"));
    EXPECT_EQ(-2.22507e-308, parse_real<double>("-2.22507e-308")); // underflow
    EXPECT_EQ(18446744073709551616.0, parse_real<double>("18446744073709551616")); // 2^64 (max of uint64_t + 1, force to use double)
    EXPECT_EQ(-9223372036854775809.0, parse_real<double>("-9223372036854775809")); // -2^63 - 1(min of int64_t + 1, force to use double)
...
TEST(ParserTest, Numbers_Should_CheckForBadInput)
{
    EXPECT_ANY_THROW(parse_integer<int>("a")); // JSON doesn't support hex or letters

    EXPECT_ANY_THROW(parse_integer<int>(".1")); // JSON needs a leading digit

    EXPECT_ANY_THROW(parse_integer<int>("1.")); // JSON needs a digit after decimal

    EXPECT_ANY_THROW(parse_integer<int>("1e")); // JSON needs a digit in exponent
}


TEST(ParserTest, Strings_Should_Parse)
{
    ...
    EXPECT_EQ("\xF0\x9D\x84\x9E", parse_string("\"\\uD834\\uDD1E\""));  // G clef sign U+1D11E
    ...
    EXPECT_EQ(u"\"\\/\b\f\n\r\t", parse_string(u"\"\\\"\\\\/\\b\\f\\n\\r\\t\""));
    EXPECT_EQ(u"\u0024",          parse_string(u"\"\\u0024\""));
    EXPECT_EQ(u"\u00A2",          parse_string(u"\"\\u00A2\"")); // Cents sign U+00A2
    EXPECT_EQ(u"\u20AC",          parse_string(u"\"\\u20AC\"")); // Euro sign U+20AC
    EXPECT_EQ(std::u16string({0xD834, 0xDD1E}),    parse_string(u"\"\\uD834\\uDD1E\"")); // G clef sign U+1D11E
    ...
    EXPECT_EQ(U"\u00A2",          parse_string(U"\"\\u00A2\"")); // Cents sign U+00A2
    EXPECT_EQ(U"\u20AC",          parse_string(U"\"\\u20AC\"")); // Euro sign U+20AC
    EXPECT_EQ(U"\U0001D11E",      parse_string(U"\"\\uD834\\uDD1E\"")); // G clef sign U+1D11E
}


TEST(ParserTest, ParseString_Transcoding)
{
    ...
    json::detail::parser_impl<stream_type, Handler, utf8, utf16> parser(std::move(stream), handler);
    parser.parse_string();
...
TEST(ParserTest, Parser_Should_ParseAroundEncodingErrors)
{
    EXPECT_THROW(parse_any("[\"\\a\"]"),
                 json::unknown_escape_character);

    EXPECT_THROW(parse_any("[\"\\uABCG\"]"),
                 json::incorrect_hex_digit);
...
TEST(ParserTest, Parser_Should_ParseEmptyArray)
{
    const std::string str("[ ] ");
    
    typedef numeric_array_handler<int, utf8> Handler;
    Handler handler;
    auto parser = json::detail::make_parser_impl(str.begin(), str.end(), handler);
    parser.parse();
...
TEST(ParserTest, Parser_Should_DetectMalformedArrays)
{
    // Must be a comma or ']' after an array element.
    EXPECT_THROW(parse_array<int>("["), std::range_error);
    EXPECT_THROW(parse_array<int>("[}"), std::range_error);
    EXPECT_THROW(parse_array<int>("[1 2]"), json::expected_comma_or_close_bracket);
}


TEST(ParserTest, Parser_Should_ParseObject)
{
    const std::string str =
        "{ "
            "\"string\" : \"hello\", "
    ...
    EXPECT_EQ(std::make_pair(std::size_t(1), std::string("hello")),  handler.strings[0]);
...
TEST(ParserTest, Parse_EmptyObject)
{
    const std::string str("{ } ");
    const auto handler = parse_object(str);
...
TEST(ParserTest, Parser_Shold_DetectObjectErrors)
{
    ...
    EXPECT_THROW(parse_object("{\"a\":1]"), json::expected_comma_or_close_curly_brace);
}


TEST(ParserTest, Parser_Should_DectectFormatErrors)
{
